...
module.exports = {
  ...
  buildAdminFixtures: app => {
    return app.services.FootprintService.find('user', {}).then(users => {
  ...
  loadAdmin: app => {
    return app.services.PassportService.register({
      username: app.config.proxyPermissions.defaultAdminUsername,
      password: app.config.proxyPermissions.defaultAdminPassword
    })
      .then(admin => {
        return global.app.services.PermissionService.addRoleToUser(admin, 'admin')
      })
  }
